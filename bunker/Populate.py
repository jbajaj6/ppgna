...
class Populate:
    ...
    def getScore(self):
        if self.position.x > -0.5 and self.position.x < 0.5 and self.position.y < 4 and self.position.y > 2:
            # print("hi")
            return (hole_pos - self.position).magnitude()
        
        if self.position.x > 0:
            return (Vec(0.5, 4, 0) - self.position).magnitude() + (hole_pos - Vec(0.5, 4, 0)).magnitude()
        return (Vec(0.5, 4, 0) - self.position).magnitude() + (hole_pos - Vec(0.5, 4, 0)).magnitude()
