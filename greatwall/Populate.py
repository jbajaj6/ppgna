...
from WindowSingleton import WindowSingleton, ppm, hole_pos, valley_center, hill_center
...
class Populate:
    ...
    def update(self):
        ...
        valley_dist = (valley_center - self.position).magnitude()
        if not (valley_dist >= 0.2 and valley_dist <= 1):
    ...
    def getScore(self):
        total_dist = 0
        if self.position.x > -0.5 and self.position.y < hill_center.y:
            total_dist += (valley_center - self.position).magnitude()
            total_dist += (hill_center - valley_center).magnitude()
            total_dist += (hill_center - hole_pos).magnitude()
            return total_dist
        if self.position.x > 2 and self.position.x < 4 and self.position.x * 2 + 3 < self.position.y:
            total_dist += (valley_center - self.position).magnitude()
            total_dist += (hill_center - valley_center).magnitude()
            total_dist += (hill_center - hole_pos).magnitude()
