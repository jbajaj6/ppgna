...
hole_pos = Vec(-0.5, 0.5, 0)

wall_corners = (Vec(0, 0, 0), Vec(0, 3, 0))
...
class WindowSingleton(object):
    def __new__(cls):
        if not hasattr(cls, 'instance'):
            ...
            wall = Line(Point((3.5+wall_corners[0].x) * ppm, (7-wall_corners[0].y) * ppm), Point((3.5+wall_corners[1].x) * ppm, (7-wall_corners[1].y) * ppm))
            wall.draw(cls.win)
        return cls.instance
